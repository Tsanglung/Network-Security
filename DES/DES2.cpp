...
void generateKeys()
{
    bitset<56> realKey;//輸入金鑰
    bitset<28> left;//儲存前28位
    bitset<28> right;//儲存後28位
    bitset<48> compressKey;//儲存壓縮的48位元金鑰
    for(int i=0; i<56;++i)// 去掉奇偶標記位元，將64位元金鑰->56位
        realKey[55-i]= key[64-PC_1[i]];
    for(int round=0; round<16;++round)// 生成子金鑰，subKeys[16]中
    {
		// 前28位
        for(int i=28; i<56;++i)
            left[i-28]= realKey[i];
        ...
        subKey[round]= compressKey;//得到回合金鑰
    }
    for(int round=0; round<16;++round)
    {
        string s;
        s=subKey[round].to_string();
        int l=s.size();
	    cout<<round+1<<"(round key) : ";
	        for(int i=0;i<l;i+=4){
...
int main(){
    ...
    string s = "123456ABCD132536";
	string k = "0123ABCD25621456";
